...
class TablingStoreAvailabilityView extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _TablingStoreAvailabilityView();
}

class _TablingStoreAvailabilityView
    extends State<TablingStoreAvailabilityView> {
  final ApiService _apiService = ApiService();
  late Future<dynamic> _vacancyListFuture;
  ...
  void _initData() {
    _vacancyListFuture = _getVacancyList();
  }

  Future<dynamic> _getVacancyList() {
  ...
  FutureOr<dynamic> _updateVacancyList() {
    Provider.of<Platform>(context, listen: false).isLoading = true;
    _vacancyListFuture = _getVacancyList();
    setState(() {});
  }

  void _onTapReply(String vacancyId) {
    context
        .pushNamed('table_answer_view', extra: vacancyId)
        .then((value) => _updateVacancyList());
  }
                  ...
                  if (snapshot.data != null) {
                    return Container(
                        ...
                        child: Column(children: [
                          ...
                          BottomButtons(
                              btn3Enabled: true,
                              btn3Text: '상점 예약내역 바로가기',
                              btn3Color: Colors.black,
                              btn3Pressed: () =>
                                  context.goNamed('reserv_store_avail')),
                        ]));
                  } else {
